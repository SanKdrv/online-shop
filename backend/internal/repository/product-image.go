...
func (r *ProductsImagesRepo) GetImageHashesByProductId(productId int64) ([]string, error) {
	var hashes []string
	if err := r.db.Model(&domain.ProductImage{}).Where("product_id = ?", productId).Order("order ASC").Pluck("image_hash", &hashes).Error; err != nil {
		return []string{}, err
	}
	return hashes, nil
}
...
func (r *ProductsImagesRepo) UpdateProductImage(oldName string, newName string) error {
	return r.db.First(&domain.ProductImage{}, "image_hash = ?", oldName).Update("image_hash", newName).Error
}
